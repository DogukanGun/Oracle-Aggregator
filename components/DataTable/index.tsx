...
import LoadingError, { LoadingErrorPropsType } from '../LoadingError';
...
const NoDataBody = ({
  loading,
  error,
}: Pick<LoadingErrorPropsType, 'loading' | 'error'>) =>
  loading || error ? (
...
const NoDataText = () => {
  ...
  return (
    <Stack className="items-center">
      <Typography variant="caption3" text={text} />
    </Stack>
  );
};
...
const Header = () => {
  const { cols, rounded, stickyHeader } = useTableContext();
  const roundedBorder = rounded ? 'rounded-sm' : '';
  const headerPos = stickyHeader ? 'sticky top-0 z-10' : '';

  const classes = [roundedBorder, headerPos].join(' ');
  ...
  return (
    <tbody>
      {!loading && !error && sortedData && sortedData.length > 0 ? (
        sortedData.map((data, index) => <Row {...{ data, index }} />)
      ) : (
        <td colSpan={100} className="p-2 py-4">
          <NoDataBody loading={loading} error={error?.message} />
        </td>
      )}
    </tbody>
  );
...
const Row = <T,>({ data, index }: TableRowPropsType<T>) => {
  ...
  const classes = useMemo(() => {
    const rowStyleClasses = rowStyle ? ' ' + rowStyle(data, index) : '';
    let base = '';
    if (onRowClick)
      base +=
        'cursor-pointer hover:bg-surface-dark dark:hover:bg-surfaceDark-dark';
    else
      base += 'border-b mt-1 dark:border-surfaceDark-dark border-surface-dark';
    return base + rowStyleClasses;
  }, [onRowClick, rowStyle, data, index]);
