...
const CardDialog = ({
  ...
  const classes = useMemo(() => {
    const base = ['p-4 w-full max-h-full m-4 lg:p-8 lg:w-1/2 lg:m-0 max-w-2xl'];
    if (className) base.push(className);
    return base.join(' ');
  }, [className]);

  const _contentWrapperClasses = useMemo(() => {
    const base = ['overflow-y-auto max-h-[60vh]'];
    if (contentWrapperClasses) base.push(contentWrapperClasses);
    return base.join(' ');
  }, [contentWrapperClasses]);
    ...
    >
      <Card elevation={0} className={classes}>
        <Stack>
          ...
          <div className={_contentWrapperClasses}>
